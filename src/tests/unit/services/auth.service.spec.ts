...
describe("AuthService", () => {
  ...
  describe("register", () => {
    ...
    it("should successfully register a new user", async () => {
      ...
      expect(jwt.sign).toHaveBeenCalledWith({ id: mockUserId }, mockJwtSecret, {
        expiresIn: "7d",
      });
  ...
  describe("getProfile", () => {
    ...
    it("should throw error when user is not found", async () => {
      ...
      await expect(authService.getProfile(mockUserId)).rejects.toThrow(
        "User not found"
      );
  ...
  describe("generateToken (private method)", () => {
    ...
    it("should use same token generation for login", async () => {
      ...
      (bcrypt.compare as jest.Mock).mockResolvedValue(true); // ✅ mock password check
      (jwt.sign as jest.Mock).mockReturnValue(mockToken);
